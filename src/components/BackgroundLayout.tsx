...
const AuthLayout: React.FC<AuthLayoutProps> = ({ children, showLeftPanel = true }) => {
  ...
  return (
    <div className="min-h-screen flex relative">
      ...
      >
        <HomeOutlined style={{ color: "#FACC15", fontSize: 24 }} />
      </div>
      ...
      {showLeftPanel && (
        <div className="w-1/2 hidden md:flex flex-col items-center justify-center bg-[#0F3952] relative">
          ...
          <div className="relative flex flex-col items-center px-6 text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-2">Welcome to Yosti</h1>
            <p className="text-white text-lg md:text-xl">Your gateway to seamless import-export solutions</p>
          </div>
        </div>
      )}

      <div className={`flex items-center justify-center p-6 ${showLeftPanel ? "w-full md:w-8/12" : "w-full"}`}>
